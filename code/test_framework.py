...
# Use Agg backend for matplotlib to avoid issues with GUI backends
matplotlib.use('TkAgg')

# Get the current working directory
path = os.getcwd()

# Define the path to the trained YOLO model and dataset
model_path = "./models/yolo11_n/weights/best.pt"
dataset_path = "./FLAME-T/Point D/"
...
# Get a list of all image files with .jpeg extension in the image directory
list_of_images = glob.glob(f"{img_path}*.jpeg")

# Create a list of corresponding label file paths by replacing 'images' with 'labels' in the file paths
list_of_labels = [image.replace('images', 'labels').replace('.jpeg', '.txt') for image in list_of_images]
